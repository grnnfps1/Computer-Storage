...
import com.computerstorage.common.transfer.TransferProgramValidator;
...
public final class LogisticsManager implements IComputerService {
    ...
    @Override public void tick(Computer computer) {
        if (level == null) { lastMoved = 0; return; }
        if (runtime == null) runtime = new ControllerLogisticsRuntime(
                new TransferProgramRepository(computer.transferPrograms()), id -> endpoints.resolve(id, level));
        lastMoved = runtime.tick(level.getGameTime(), false);
    }

    public void bindLevel(ServerLevel level) { this.level = level; this.runtime = null; }
    ...
    public TransferProgramValidator.Validation validateProgram(TransferProgram program) {
        return TransferProgramValidator.validate(program, endpoints);
    }

    public boolean addProgram(Computer computer, TransferProgram program) {
        if (!validateProgram(program).valid()) return false;
        if (computer.transferPrograms().programs().size() >= TransferProgramValidator.MAX_PROGRAMS) return false;
        boolean added = computer.transferPrograms().add(program);
        if (added) runtime = null;
        return added;
    }

    public boolean removeProgram(Computer computer, String id) {
        boolean removed = computer.transferPrograms().remove(id);
        if (removed && runtime != null) runtime.clearRuntime(id);
        return removed;
    }
}
