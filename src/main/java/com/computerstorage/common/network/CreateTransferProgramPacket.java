...
public record CreateTransferProgramPacket(
    ...
    public void encode(FriendlyByteBuf buf) {
        ...
        buf.writeInt(maxItemsPerOperation);
        buf.writeInt(minSourceAmount);
        buf.writeInt(maxDestinationAmount);
        buf.writeEnum(condition);
        buf.writeLong(intervalTicks);
        buf.writeLong(offsetTicks);
        buf.writeBoolean(enabled);
    }

    public static CreateTransferProgramPacket decode(FriendlyByteBuf buf) {
        return new CreateTransferProgramPacket(
                NetworkPackets.readString(buf, MAX_ID_LENGTH),
                NetworkPackets.readString(buf, MAX_ID_LENGTH),
                NetworkPackets.readString(buf, MAX_ID_LENGTH),
                buf.readEnum(TransferFilter.Mode.class),
                NetworkPackets.readString(buf, MAX_ITEM_ID_LENGTH),
                buf.readInt(), buf.readInt(), buf.readInt(), buf.readInt(),
                buf.readEnum(TransferCondition.class), buf.readLong(), buf.readLong(), buf.readBoolean());
    }

    public TransferProgram toProgram() {
        return new TransferProgram(id, sourceId, destinationId,
                new TransferFilter(filterMode, filterItemId), priority,
                maxItemsPerOperation, minSourceAmount, maxDestinationAmount,
                condition, new TransferSchedule(intervalTicks, offsetTicks));
    }
}
